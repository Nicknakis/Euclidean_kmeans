...
class Euclidean_Kmeans():
    def __init__(self,cond_control,k_centers,dimensions,init_cent=None,split_mask=None,previous_cl_idx=None,full_prev_cl=None,prev_centers=None,full_prev_centers=None,centroids_split=None,assigned_points=None,aux_distance=None,local_idx=None,initialization=1,device= None , n_iter=300,tol=1e-04):
        """
...
# =============================================================================
                indicator=torch.cat([torch.arange(centroids_split.sum().long(),device=self.device).unsqueeze(0),torch.zeros(centroids_split.sum().long(),device=self.device).long().unsqueeze(0)],0)
           
                            
                centers_indicator=torch.cat([torch.arange(self.previous_cl_idx.shape[0],device=self.device).unsqueeze(0),self.previous_cl_idx.unsqueeze(0)],0)
                
                indexC, valueC = spspmm(centers_indicator,torch.ones(centers_indicator.shape[1],device=self.device), indicator,self.condensed_centers,self.previous_cl_idx.shape[0],centroids_split.shape[0],1,coalesced=True)
...
#=============================================================================
    ...
    def fit(self,Data,Data_grad=None):
        '''
        '''
        ...
        for t in range(300):
            if t==0:
                ...
                self.Kmeans_step(Data)
                if self.pdist_tol(self.previous_centers,self.centroids).sum()<self.tol:
                    break
                self.previous_centers=self.centroids
